...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async processPayment() {    //send the payment information to Laravel + stripe
                this.paymentProcessing = true;  //disables all form buttoms
                ...
                if (error) {
                    ...
                    this.customer.amount = this.$store.state.cart.reduce((acc, item) => acc + (item.price * item.quantity), 0); //calculate total using reduce
                    this.customer.cart = JSON.stringify(this.$store.state.cart);

                    axios.post('/api/purchase', this.customer)
                        .then((response) => {
                            this.paymentProcessing = false; //form field no longer disabled
